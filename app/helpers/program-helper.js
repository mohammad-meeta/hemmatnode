...
ProgramHelper.updateProgramData = async function updateProgramData(data) {
    const Program = mongoose.model('Program');
    let res2 = Program.findByIdAndUpdate(data._id, data, { useFindAndModify: false, new: true });
    const pipeline = [
        {
            $match: {
                _id: res2._id,
 ...
 * update
 */
ProgramHelper.updateProgramData = async function updateProgramData(
    data
) {
    const Program = mongoose.model("Program");

    let res2 = await Program.findByIdAndUpdate(data._id, data, {
