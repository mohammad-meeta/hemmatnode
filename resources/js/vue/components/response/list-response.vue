    ...
    table.table.is-striped.is-hoverable.is-fullwidth(v-if="hasResponse")
        ...
        tbody
            tr(v-for='response in responses', :key='response._id')
                td {{ response.title }}
                td {{ response.is_active }}
                td {{ toPersianDate(response.created_at) }}
                td.function-links
                    a.button.is-primary.is-rounded(href="#", @click.prevent="commandClick(ENUMS.COMMAND.EDIT, response)")
                        span.icon.is-small
                            i.material-icons.icon check_circle
                        span ویرایش
                    a.button.is-warning.is-rounded.mt-2(href="#", @click.prevent="commandClick(ENUMS.COMMAND.SHOW, response)")
                        span.icon.is-small
